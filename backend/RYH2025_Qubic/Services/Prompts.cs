...
Has Input Parameters: {(method.InputStruct.Any() ? "YES" : "NO")}
Input Count: {method.InputStruct.Count}
{(method.InputStruct.Any() ?
            $"Available Parameters:\n{string.Join("\n", method.InputStruct.Select(kv => $"- {kv.Key}: {kv.Value}"))}" :
            "- NO INPUT PARAMETERS AVAILABLE")}

MALICIOUS VALUE MAPPINGS (use ONLY if targeting an actual input parameter):
- uint8: ""255"" (MAX_UINT8)
...
For methods WITH input parameters:
[
  {{
...
For methods WITHOUT input parameters:
[
  {{
    ""testName"": ""Internal Logic Vulnerability"",
    ""methodName"": ""{method.Name}"",
    ""targetVariable"": ""internal_state"",
    ""description"": ""Tests internal logic vulnerabilities in {method.Name} which has no external inputs"",
...
GENERATE TEST CASES:
- If method has input parameters: Generate 1-3 tests targeting ACTUAL input parameters
- If method has NO input parameters: Generate 1 internal logic test with empty testInputs
- NEVER create tests for non-existent parameters
- Always use actual parameter names from the method signature";
        }
